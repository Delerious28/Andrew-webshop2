...
export function ProductCard({ product, accent, showNewBadge }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className={`group card-surface p-5 flex flex-col gap-4 transition duration-300 hover:-translate-y-1 ${accent ? 'border-brand/40 shadow-xl' : ''}`}
    >
      <div className="relative w-full aspect-[4/3] overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        {firstImage ? (
          <Image src={firstImage} alt={product.title} fill className="object-contain transition duration-500 group-hover:scale-105" />
        ) : (
      ...
      <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-300">
        <div className="flex items-center gap-2">
          ...
          <span>{stockCopy}{product.stock > 0 ? ` · ${product.stock} available` : ''}</span>
        </div>
